...
function renderMenuItems() {
    ...
    // Filter by diet
    if (state.activeDietFilter === 'veg' || state.activeDietFilter === 'jain') {
        items = items.filter(i => i.isVeg);
    ...
    const customJainCardHtml = isJainActive ? `
        <div class="sm:col-span-2 lg:col-span-3 p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-[#24140b] via-[#331c11] to-[#24140b] border-2 border-emerald-500/60 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-white mb-2">
            <div class="flex items-start sm:items-center gap-4">
                <div class="w-12 h-12 rounded-xl bg-emerald-600/30 text-emerald-400 border border-emerald-500/40 flex items-center justify-center flex-shrink-0 mt-0.5 sm:mt-0">
                    <i data-lucide="leaf" class="w-6 h-6"></i>
                </div>
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        <span class="bg-emerald-600/40 text-emerald-300 text-[10px] font-bold px-2 py-0.5 rounded-full border border-emerald-500/50 uppercase tracking-wider">
                            🌿 Custom Jain Kitchen Preparation
                        </span>
                    </div>
                    <h3 class="font-serif-title text-lg sm:text-xl font-bold text-white">
                        Need Custom Jain Dishes (No Onion, No Garlic)?
                    </h3>
                    <p class="text-xs text-neutral-300 leading-relaxed mt-0.5 max-w-2xl">
                        Looking for 100% strict Jain preparation or a custom recipe? We freshly prepare pizzas, handcrafted pastas, wraps, starters, and beverages to your exact dietary preferences.
                    </p>
                </div>
            </div>
            <a href="${CAFE_INFO.whatsappJainInquiryUrl}" target="_blank" rel="noopener" class="btn-amber-glow text-black font-bold text-xs uppercase tracking-wider px-5 py-3 rounded-xl flex items-center gap-2 whitespace-nowrap shadow-lg flex-shrink-0">
                <svg class="w-4 h-4 inline-block" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
                <span>Request Custom Jain Item</span>
            </a>
        </div>
    ` : '';

    if (items.length === 0) {
        container.innerHTML = customJainCardHtml;
